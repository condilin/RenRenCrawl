...
class RenRenCrawl(object):
    """
    建议将出错的url保存到一个文件内，最后再对这些url进行抓取。
    """
    def __init__(self):
        # 列表信息url
    ...
    def __del__(self):
       ...
       :return:
        ...
        print('程序结束，一共耗时%s秒' % (time.time() - self.start_time))
    ...
    def get_web_source(self):
        ...
        """
        # 循环获取内容
        for i in range(1, 8):  # 2785540
            # 发送请求，获取响应
            response = requests.get(
                # 定义请求 url
                self.__info_detail.format(i),
                # 定义请求头
    ...
    def clean_data(self):
        ...
        清洗数据
        :return: res_dict：
            a variable which type of dict, clean the web source
        """
        # 通过正则匹配获取用户信息
        ser_info = re.search(r"var info = \'(.*)\'", self.get_web_source()).group(1)
    ...
    def save_data(self):
        ...
        info['borrower']['age'] = str(datetime.now().year - int(
            jsonpath(res_dict, '$.borrower.birthDay')[0][:4]))  # 年龄=当前时间-出生年月
        info['borrower']['graduation'] = jsonpath(res_dict, '$.borrower.graduation')[0]  # 学历
        info['borrower']['marriage'] = '已婚' if jsonpath(res_dict, '$.borrower.marriage')[
                                                   0] == 'MARRIED' else '未婚'  # 婚姻
        ...
        info['borrower']['house_loan'] = '有房贷' if jsonpath(res_dict, '$.borrower.houseLoan')[
            0] else '无房贷'  # 房贷
        ...
        info['borrower']['car_loan'] = jsonpath(res_dict, '$.hasOthDebt')[0] if \
        jsonpath(res_dict, '$.hasOthDebt')[0] else '无'  # 其他负债

        # ------------------ 信用信息 -------------------- #
        info['userLoanRecord']['total_count'] = str(
            jsonpath(res_dict, '$.userLoanRecord.totalCount')[0]) + '笔'  # 申请借款
        info['userLoanRecord']['available_credits'] = str(
            jsonpath(res_dict, '$.borrower.availableCredits')[0]) + '元'  # 信用额度
        info['userLoanRecord']['overdue_total_amount'] = str(
            jsonpath(res_dict, '$.userLoanRecord.overdueTotalAmount')[0]) + '元'  # 逾期金额
        info['userLoanRecord']['success_count'] = str(
            jsonpath(res_dict, '$.userLoanRecord.successCount')[0]) + '笔'  # 成功借款
        info['userLoanRecord']['borrow_mount'] = str(
            jsonpath(res_dict, '$.userLoanRecord.borrowAmount')[0]) + '元'  # 借款总额
        info['userLoanRecord']['overdue_count'] = str(
            jsonpath(res_dict, '$.userLoanRecord.overdueCount')[0]) + '次'  # 逾期次数
        info['userLoanRecord']['already_pay_count'] = str(
            jsonpath(res_dict, '$.userLoanRecord.alreadyPayCount')[0]) + '笔'  # 还清笔数
        info['userLoanRecord']['notpay_total_amount'] = str(
            jsonpath(res_dict, '$.userLoanRecord.notPayTotalAmount')[0]) + '元'  # 待还本息
        info['userLoanRecord']['failed_count'] = str(
            jsonpath(res_dict, '$.userLoanRecord.failedCount')[0]) + '笔'  # 严重逾期????????????????
        ...
        self.db.info_set1.insert_one(info)
